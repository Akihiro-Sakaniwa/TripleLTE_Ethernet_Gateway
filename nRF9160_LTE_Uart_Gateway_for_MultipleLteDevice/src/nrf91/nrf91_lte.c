...
volatile int edrx_remain;

static uint8_t cyc[] = "|-\\|/-\\";

uint8_t my_addr[4] = {0,0,0,0};
...
static volatile bool is_cgev_act_on = false;
static volatile bool is_cgev_detach_on = false;
bool is_cgev_act(void)
{
	return is_cgev_act_on;
}
bool is_cgev_detach(void)
{
	return is_cgev_detach_on;
}
...
static int nrf91_lte_connect(bool is_init)
{
	...
	if (v1 != 0 && v1 != 4) 
	{
		if (0 != nrf91_lte_disconnect(is_init ? 0 : 4))
		{
			//return -2;
		}
	}
	...
	if (0 != nrf_modem_at_printf("AT+COPS=1,2,\"%d\"",CARRIER_1))
	{
		;;
	}
...
static void nrf91_main_loop_lte(int (*cbf)(void))
{
	...
	if (v1 == 0 || v1 == 4)
	{
		...
		{
			int ret = nrf91_lte_connect(true);	// 電波状況が悪いと時間がかかる処理
			if (ret == 0) {
				;;
			} else if (ret >= -4) {
	...
	while (true)
	{
		...
		while (true)
		{
			...
			*(uint32_t*)my_addr = my_ip_addr.s_addr;
			break;
		}
		if (psm_mode ==1)
		{
